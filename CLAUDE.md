...
- **RTS waveform timings are fixed.** Timing constants in `src/rts/waveform.rs` are derived from the Somfy patent (US7860481 B2: 1280µs bit period) and cross-checked against Pi-Somfy. The only frame-count knob is `Command::ProgLong` (CLI `--long` → wire `prog_long`; HTTP/WS `{"command":"prog_long",…}`), which routes to `waveform::build_long` (`FRAME_COUNT_LONG = 20`) so the Pi can act as the master and put the motor into pair-listen. Up/Down/Stop and short `Prog` always use `FRAME_COUNT = 4`. Do not re-add a generic `frame_count` config knob.
...
- **Switching drivers post-install** is `somfy config set-driver <kind>`: rewrites `/etc/somfy/config.toml`, re-runs `prepare_rts_prereqs()` if switching to `rts`, and restarts the unit. There is no `somfy config validate` subcommand — validation runs implicitly on every `config::resolve()` (so on startup, install, doctor, and set-driver).
